    ...
    class ASSetupAssistantCoreService(ASCoreServiceRepresentative):
        ...
        def startSetup(self, express=True, disclaimer=None):
            ...
            if not express:
                ...
                self.runStep("Знайте свои права", "AliceOS является свободным программным обеспечением с открытым исходным кодом, лицензированным по условиям лицензии BSD 2-го типа. Эта лицензия позволяет вам и создателю игры модифицировать AliceOS в любом ключе без необходимости спрашивать разрешения.\n\nЭтот тип лицензии BSD должен быть включён в пакет AliceOS; в противном случае, посетите сайт {b}https://opensource.org/licenses/BSD-2-Clause{/b} и свяжитесь с разработчиком игры, дабы тот добавил лицензию.")
            if disclaimer != None:
